...
class ChannelAggregationFFN(nn.Module):
    """An implementation of FFN with Channel Aggregation.带有通道聚合的前馈神经网络(FFN)
    ...
    Args:
        ...
        参数：
        embed_dims (int): 特征维度，与 `MultiheadAttention` 相同。
        feedforward_channels (int): FFN 的隐层维度。
        kernel_size (int): 作为深度卷积的深度可分离卷积核大小。默认为3。
        act_type (str): 激活函数的类型。默认为'GELU'。
        ffn_drop (float, 可选): 在 FFN 中将一个元素设为0的概率。默认值为0.0。
    """
...
class MultiOrderDWConv(nn.Module):
    """Multi-order Features with Dilated DWConv Kernel. 使用空洞可分离卷积核的多阶特征。
...
class MogaBlock(nn.Module):
    ...
    Args:
        ...
        embed_dims：输入通道数。
        ffn_ratio：前向网络中隐藏层通道数的扩展比例，默认为4。
        drop_rate：嵌入后的dropout概率，默认为0。
        drop_path_rate：随机深度率，默认为0.1。
        act_type：投影和FFN的激活类型，默认为"GELU"。
        norm_cfg：归一化层的类型，默认为"BN"。
        init_value：层缩放的初始值，默认为1e-5。
        attn_dw_dilation：三个DWConv层的膨胀因子列表。
        attn_channel_split：分离通道的相对比例列表。
        attn_act_type：门控分支的激活类型，默认为"SiLU"。
    """
...
class MogaNet(nn.Module):
    ...
    Args:
        ...
        arch (str): MogaNet 的结构，可以是 'tiny'、'small'、'base' 或 'large'。默认为 'tiny'。
        in_channels (int): 输入通道数。默认为 3。
        num_classes (int): 线性分类器的类别数。默认为 1000。
        drop_rate (float): 嵌入后的 Dropout 比率。默认为 0。
        drop_path_rate (float): 随机深度比率。默认为 0.1。
        init_value (float): 初始化 Layer Scale 的值。默认为 1e-5。
        head_init_scale (float): 分类器初始化缩放因子，用于高分辨率微调。默认为 1。
        patch_sizes (List[int | tuple]): Patch Embedding 中的补丁大小。默认为 [3, 3, 3, 3]。
        stem_norm_type (str): 检干的归一化层的类型。默认为 'BN'。
        conv_norm_type (str): 卷积归一化层的类型。默认为 'BN'。
        patchembed_types (list): 每个阶段的 PatchEmbedding 的类型。默认为 ['ConvEmbed', 'Conv', 'Conv', 'Conv']。
        attn_dw_dilation (list): Moga Blocks 中深度卷积核的膨胀率。默认为 [1, 2, 3]。
        attn_channel_split (list): Moga Blocks 中三个深度卷积核的通道分裂比例。默认为 [1, 3, 4]，其中[1/8, 3/8, 4/8]。
        attn_act_cfg (dict): Moga Blocks 中门控激活的配置字典。默认为 dict(type='SiLU')。
        attn_final_dilation (bool): 是否采用膨胀的深度卷积核进行最终处理。默认为 False。
        attn_force_fp32 (bool): 是否强制门控模块使用 FP32 运行。警告：如果在训练期间使用 'attn_force_fp32'=False，则在评估期间也应保持为false，因为是否使用 'attn_force_fp32' 的输出结果不同。我们在此库中将其设置为false，以方便代码迁移。默认为 False。
        fork_feat (bool): 是否为密集预测任务输出4个阶段的特征。默认为 False。
        frozen_stages (int): 要冻结的阶段（停止梯度并设置评估模式），-1表示不冻结任何参数。默认为 -1。
        init_cfg (dict): 用于mmdetection和mmsegmentation模型的初始化配置字典，用于加载预训练权重。默认为None。
        pretrained (str): mmdetection和mmsegmentation模型用于加载预训练权重（旧版本）的路径。默认为None。
    """
    arch_zoo = {
...
model = MogaNet(num_classes=8105)
# print(model)
count(model)