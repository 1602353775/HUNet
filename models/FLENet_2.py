...
class PatchEmbed_64(nn.Module):

    def __init__(self, in_chans, embed_dim, act_layer,norm_layer):
        super().__init__()
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=3, stride=1,padding=1, bias=False)
        if norm_layer is not None:
            self.norm = norm_layer(embed_dim)
        else:
            self.norm = nn.Identity()
...
class FLENet(nn.Module):
    ...
    def __init__(self,
        ...
        assert attention in ('SE', 'SK', 'CBAM', 'CAM', 'SAM','SBAM', 'CA', 'ECA', 'PA','SimAM','NUM', 'RFAConv','Moga')
        ...
        # self.patch_embed = PatchEmbed_64(
        #     in_chans=in_chans,
        #     embed_dim=embed_dim,
        #     act_layer = act_layer,
        #     norm_layer=norm_layer if self.patch_norm else None
        ...
        if attention == 'ECA':
            ...
            attention_layer = simam_module(channels=embed_dim * 2 ** (self.num_local_stages-1))
        else:
            attention_layer = Channel_Att(channels=embed_dim * 2 ** (self.num_local_stages-1))  # NUM
