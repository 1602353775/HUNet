...
class Heads_FLENet(nn.Module):
    ...
    def __init__(self,
        ...
        self.classes_per_gropp = classes_per_group
        ...
        # 汉字等级分类头 包括多个预定义等级和一个开放类，等级划分根据实际数据量
        self.hidden_feature_dim = 128
        
        self.level_feature = nn.Sequential(
            nn.Conv2d(self.last_channels,self.hidden_feature_dim,3,1,1),
            nn.AdaptiveAvgPool2d(1)
        )
        
        self.level_classifier = nn.Linear(self.hidden_feature_dim, self.group_num+1)
        ...
        # 多个分类头用于识别不同等级的汉字
        self.multi_classifier = nn.ModuleList([nn.Linear(feature_dim, classes_per_group[i]) for i in range(self.group_num)])
    ...
    def forward(self, x, group_labels=None,train=False):
        ...
        if train:
            ...
            if self.Pretraining:  # 在整个数据集上训练一个分类头
                ...
                group_feats = self.level_feature(x)
                group_feats = torch.flatten(group_feats, 1)
                group_logits = self.level_classifier(group_feats)
                group_logits = self.pos_drop(group_logits)
                ...
                for g in range(self.group_num):
                    ...
                    g_feats = feats[pos_mask, :]
                    g_logits = self.multi_classifier[g](g_feats)
                    g_logits = self.pos_drop(g_logits)
                    multi_logits.append(g_logits)
                
                return group_logits,multi_logits
        else:
            group_feats = self.level_feature(x)
            group_feats = torch.flatten(group_feats, 1)
            grops_logits = self.level_classifier(group_feats)
            group_preds = torch.max(grops_logits, dim=1)[1]

            preds_labels = torch.empty(x.size(0)).to('cuda')
            feats = self.avgpool_pre_head(x)
            feats = torch.flatten(feats, 1)
            for g in range(self.group_num+1):
                pos_mask = group_preds == g
                if pos_mask.sum()>0:
                    if g == self.group_num:
                        preds_labels[pos_mask]=0  
                    else:
                        g_feats = feats[pos_mask, :]
                        g_logits = self.multi_classifier[g](g_feats)
                        g_preds = torch.max(g_logits, dim=1)[1]
                        g_preds = g_preds.to(preds_labels.dtype)
                        preds_labels[pos_mask] = g_preds
            pred = torch.stack([group_preds, preds_labels], dim=1)
            
            return pred
            
def Heads_FLENet_T0(num_classes):
    model = Heads_FLENet(in_chans=3,
            img_size=128,
...
def Heads_FLENet_T1(num_classes):
    model = Heads_FLENet(in_chans=3,
            img_size=128,
            num_classes=num_classes,
            embed_dim=40,
...
def Heads_FLENet_T2(num_classes):
    model = Heads_FLENet(in_chans=3,
            img_size=128,
            num_classes=num_classes,
            embed_dim=60,
...
if __name__ == '__main__':
    ...
    model = Heads_FLENet(num_classes=20)
    group_logits,multi_logits = model(x,groups,train=True)
    print(group_logits.shape,multi_logits[0].shape,multi_logits[1].shape,multi_logits[2].shape,multi_logits[3].shape)
