...
class Conv2d_BN(torch.nn.Sequential):
    def __init__(self, a, b, ks=1, stride=1, pad=0, dilation=1, groups=1, bn_weight_init=1, resolution=-10000):
        super().__init__()
        # 创建 Conv2d 层，并添加到 Sequential 容器中。
        self.add_module('c', torch.nn.Conv2d(a, b, ks, stride, pad, dilation, groups, bias=False))
        # 创建 BatchNorm2d 层，并添加到 Sequential 容器中。
        self.add_module('bn', torch.nn.BatchNorm2d(b))
        # 使用给定的常数值初始化 BN 层中的缩放系数 weight 和偏移量 bias。
        torch.nn.init.constant_(self.bn.weight, bn_weight_init)
        torch.nn.init.constant_(self.bn.bias, 0)

    # torch.no_grad() 装饰器表示该方法不需要计算梯度信息，从而提高运行速度。
    @torch.no_grad()
    def fuse(self):
        # 获取内部的 Conv2d 和 BatchNorm2d 层。
        c, bn = self._modules.values()
        # 计算标准化 BN 层参数后的权重系数 w，通过广播机制增加了一个维度。
        w = bn.weight / (bn.running_var + bn.eps) ** 0.5
        # 使用计算后的 w 权重系数乘以原卷积层的权重 coeff 重新计算权重。
        w = c.weight * w[:, None, None, None]
        # 计算偏移量 b，该值是通过缩放系数、均值和偏移量三个参数得到的。
        b = bn.bias - bn.running_mean * bn.weight / (bn.running_var + bn.eps) ** 0.5
        # 使用新的权重和偏置项 coeff 和 bias 构建新的卷积层。
        m = torch.nn.Conv2d(w.size(1) * self.c.groups, w.size(0), w.shape[2:], stride=self.c.stride,
                            padding=self.c.padding, dilation=self.c.dilation, groups=self.c.groups)
        m.weight.data.copy_(w)  # 将新的权重系数赋给之前的权重值
        m.bias.data.copy_(b)  # 将新的偏置项赋给之前的偏置值
        return m
...
#     # model = EfficientViT(num_classes=10)
#     # y = model(x)
#     att = CascadedGroupAttention(64, 16)
#     y = att(x)
#     print(y.shape)
...
#
# model = EfficientViT(num_classes=8105)
#
# # print(model)
# count(model)