...
def reed_data_font():
    ...
    test_images_path={'ks':[],'xs':[],"cs":[],"ls":[],"zs":[]}
    test_images_label={'ks':[],'xs':[],"cs":[],"ls":[],"zs":[]}
        ...
        for subfolder in os.listdir(test_dir):
            ...
            if os.path.isdir(subfolder_path):
                for image_name in os.listdir(subfolder_path):
                    ...
                    if image_path.startswith('.'):
                        continue
                    font = image_name.split('-')[0]

                    test_images_path[font].append(image_path)
                    # 标签转换
                    test_images_label[font].append(int(class_indict[subfolder]))
...
def main_test():
    ...
    batch_size = [256, 256, 256, 128, 256, 256, 256, 200, 256, 256, 64, 64, 256, 256, 128, 1024]
    ...
    images_path,images_label = reed_data_font()
    fonts = ['zs','ls','cs','xs','ks']
    for font in fonts:
        test_images_path = images_path[font]
        test_images_label = images_label[font]
        
        model_index = 0
        for key, value in model_zoo.items():
            torch.cuda.empty_cache()
            gc.collect()
            print("开始在单字体测试集：{} 上测试模型：{},测试集样本量为：{}".format(font,key,len(test_images_label)))
            # 实例化验证数据集
            ...
            if os.path.exists(weigths_path):
                ...
                model.load_state_dict(load_weights_dict, strict=False),key
                model.eval()
                ...
                for step, data in enumerate(data_loader):
                    ...
                    data_loader.desc = "[model: {}] font: {}, acc: {:.5f},Throughput-{}: {:.2f} samples/sec ".format(key,font,accu_num.item() / sample_num,device,throughput)
