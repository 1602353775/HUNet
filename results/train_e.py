...
def main(args):
    ...
    if os.path.exists("./results_e/weights") is False:
        os.makedirs("./results_e/weights")
    if os.path.exists("./results_e/best_val_checkpoint") is False:
        os.makedirs("./results_e/best_val_checkpoint")
    if os.path.exists("./results_e/val_Error statistics") is False:
        os.makedirs("./results_e/val_Error statistics")
    ...
    if args.Multi_head:
        ...
        train_images, train_labels, test_images, test_labels = read_data_1(args.train_path,args.test_path)
        train_images, train_labels = sfzd (data_dir)
        ...
        input_size = img_size[args.img_size]
        # 实例化训练数据集
        train_dataset = MyDataSet_1(images_paths=train_images,
    ...
    for key, value in model_zoo.items():
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            # validate
            if os.path.exists("./results_e/val_Error statistics/{}".format(model_name)) is False:
                os.makedirs("./results_e/val_Error statistics/{}".format(model_name))
            excel_path = "./results_e/val_Error statistics/{}/{}.xlsx".format(model_name, epoch)
            val_loss, val_acc = evaluate_one_epoch(model=model,
                                         ...
                                         count=count)
            log_dir = "./results_e/runs/{}/".format(model_name)
            ...
            if train_acc > best_train_acc:
                ...
                if os.path.exists("./results_e/weights/{}".format(model_name)) is False:
                    os.makedirs("./results_e/weights/{}".format(model_name))
                torch.save(model.state_dict(), "./results_e/weights/{}/best_train_model.pth".format(model_name))
            if val_acc > best_val_acc:
                print("正确率上升！")
                best_val_acc = val_acc
                if os.path.exists("./results_e/weights/{}".format(model_name)) is False:
                    os.makedirs("./results_e/weights/{}".format(model_name))
                torch.save(model.state_dict(), "./results_e/weights/{}/best_val_model.pth".format(model_name))
                ...
                if os.path.exists("./results_e/best_val_checkpoint/{}".format(model_name)) is False:
                    os.makedirs("./results_e/best_val_checkpoint/{}".format(model_name))

                ckpt_name = "./results_e/best_val_checkpoint/{}/best_val_checkpoint.{}".format(model_name, CHECKPOINT_EXTN)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--datasets_count', type=bool, default=True)
    parser.add_argument('--weights', type=str, default="./results/weights", help='initial weights path')
    parser.add_argument('--checkpoint', type=str, default="", help='continue training from checkpoint')  # ./results/best_val_checkpoint
